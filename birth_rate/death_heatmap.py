# -*- coding: utf-8 -*-
"""
Heatmap for US death rate per month (deaths per capita)
...
input_file = os.path.join('data', 'usa_death_1933-2015.csv')
df_death = pd.read_csv(input_file, delim_whitespace=True)
...
df_death = df_death[['Year', 'Month', 'Deaths']]
df_death = df_death[~df_death['Month'].isin(['TOT'])]
df_death.columns = ['year', 'month', 'death']
df_death['day'] = 1
...
df_death = df_death.set_index(pd.to_datetime(df_death[['year', 'month', 'day']]))

# Create one dataframe with only the datetime index, population, and death data
df_pop = df_pop['pop']
df_death = df_death['death']
df = pd.concat([df_pop, df_death], axis=1)

# Do linear interpolation between annual data to get better numbers for monthly population
df = df.interpolate(method='time')

# Calculate death rate = monthly deaths per monthly population per days in month
days_in_month = df.index.to_series().dt.daysinmonth
df['rate'] = df['death'] / df['pop'] / days_in_month * (10**6)
print('Units are deaths/month/million people')

# Break down date columns
df['month'] = df.index.to_series().dt.month
df['year'] = df.index.to_series().dt.year

# Get rid of rows without death data
df = df.loc[df.index >= df_death.index.min()]
df = df.loc[df.index <= df_death.index.max()]
...
plt.title('Monthly USA Death Rate 1933-2015')
ax.set_xlabel("Year")
ax.set_ylabel('Deaths/day per million ppl')
ax.text(728000, 15,
        'Death data: Human Mortality Database\nPopulation data: US Census Bureau\nCode: www.github.com\\aaronpenne\nAaron Penne © 2018',
        fontsize = 8,
        color = 'gray',
        multialignment = 'right')
fig.savefig('death_rate_usa_line.png', dpi='figure', bbox_inches='tight', pad_inches=.11)
...
plt.title('USA Death Rate 1933-2015 - Month to Month comparison')
ax.set_xlabel("Month")
ax.set_ylabel('Deaths/day per million ppl')
ax.text(8, 10,
        'Death data: Human Mortality Database\nPopulation data: US Census Bureau\nCode: www.github.com\\aaronpenne\nAaron Penne © 2018',
        ...
        color = 'gray',
        multialignment = 'right')
fig.savefig('death_rate_usa_box.png', dpi='figure', bbox_inches='tight', pad_inches=.11)
...
cb = plt.colorbar(label='Deaths/day per million ppl', cax=cax)
...
# Annotations
ax.text(-0.5, -4.2,
        'Monthly USA Death Rate Per Capita 1933-2015',
        fontsize = 14,
        color = 'black',
        weight = 'bold',)
ax.text(-0.5, -2,
        'Rate = Deaths / Population / Days in Month',
        fontsize = 12,
        color = 'black',)
ax.text(64, -2,
        'Death data: Human Mortality Database\nPopulation data: US Census Bureau\nCode: www.github.com\\aaronpenne\nAaron Penne © 2018',
        ...
        color = 'gray',
        multialignment = 'right')
fig.savefig('death_rate_heat_usa.png', dpi='figure', bbox_inches='tight', pad_inches=.11)
...
# Annotations
ax.text(-0.5, -3.5,
        'Monthly USA Death Rate Per Capita ' + str(min_year) + '-' + str(max_year),
        fontsize = 12,
        color = 'black',
        weight = 'bold')
ax.text(-0.5, -2,
...
ax.text(34, -2,
        ...
        color = 'gray',
        multialignment = 'right')
fig.savefig('death_rate_usa_heat_ranged.png', dpi='figure', bbox_inches='tight', pad_inches=.11)
